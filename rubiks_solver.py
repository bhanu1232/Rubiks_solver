...
# globals for backend cube logic
moves_list = []
last_scramble = []
f2l_list = []
step_moves_list = []
...
def scramble(moves=25):
    ...
    a = make_cube()
    if isinstance(moves, str):  # scramble given a list of moves
        m(moves)
        moves_list = []
        solution_length = 0
        temp = moves.split(' ')
...
def isTopSolved():
    ...
    if a[0][0][0] == a[0][0][1] == a[0][0][2] == a[0][1][0] == a[0][1][1] == a[0][1][2] == a[0][2][0] == a[0][2][1] == \
            a[0][2][2]:
        return True
    else:
        return False
...
def isSolved():
    uside = a[0][0][0] == a[0][0][1] == a[0][0][2] == a[0][1][0] == a[0][1][1] == a[0][1][2] == a[0][2][0] == a[0][2][
        1] == a[0][2][2]
    fside = a[1][0][0] == a[1][0][1] == a[1][0][2] == a[1][1][0] == a[1][1][1] == a[1][1][2] == a[1][2][0] == a[1][2][
        1] == a[1][2][2]
    rside = a[2][0][0] == a[2][0][1] == a[2][0][2] == a[2][1][0] == a[2][1][1] == a[2][1][2] == a[2][2][0] == a[2][2][
        1] == a[2][2][2]
    lside = a[3][0][0] == a[3][0][1] == a[3][0][2] == a[3][1][0] == a[3][1][1] == a[3][1][2] == a[3][2][0] == a[3][2][
        1] == a[3][2][2]
    dside = a[4][0][0] == a[4][0][1] == a[4][0][2] == a[4][1][0] == a[4][1][1] == a[4][1][2] == a[4][2][0] == a[4][2][
        1] == a[4][2][2]
    bside = a[5][0][0] == a[5][0][1] == a[5][0][2] == a[5][1][0] == a[5][1][1] == a[5][1][2] == a[5][2][0] == a[5][2][
        1] == a[5][2][2]
    return uside and fside and rside and lside and dside and bside


def solve():
    global step_moves_list, solution_length
    cross()
    simplify_moves()
...
######################################################################################################
# Below is the modernized GUI for the Cube Solver
######################################################################################################

class CubeSolverGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("rebics solver assiment of aero hack")
        self.root.geometry("720x620")  # Adjusted height
        self.root.resizable(False, False)

        # Style configuration
        self.style = ttk.Style(self.root)
        self.style.theme_use('clam')  # Other options: 'alt', 'default', 'classic'

        # Variables
        self.custom_scramble_var = tk.StringVar(value="Enter custom scramble...")
        self.scramble_label_var = tk.StringVar(value="Scramble will be displayed here.")
    ...
    def _create_widgets(self):
        ...
        self._create_solver_ui(main_frame)

    def _create_solver_ui(self, parent):
        # Left side: Canvas and rotation
        left_frame = ttk.Frame(parent)
        left_frame.pack(side=tk.LEFT, fill=tk.Y, padx=(0, 10))

        self.canvas = tk.Canvas(left_frame, width=225, height=330, background='white', highlightthickness=1)
        self.canvas.pack(pady=(0, 10))
        self.canvas.bind("<Button-1>", lambda e: self.toggle_transparency())

        rotation_frame = ttk.LabelFrame(left_frame, text="Rotate View", padding="5")
        rotation_frame.pack(fill=tk.X)

        ttk.Button(rotation_frame, text="<-- (Y)", command=lambda: self.rotate_view("y")).pack(side=tk.LEFT,
                                                                                               expand=True, fill=tk.X)
        ttk.Button(rotation_frame, text="(Y') -->", command=lambda: self.rotate_view("yi")).pack(side=tk.RIGHT,
                                                                                                 expand=True, fill=tk.X)
        ...
        # Scramble Controls
        scramble_controls_frame = ttk.LabelFrame(right_frame, text="Controls", padding="10")
        scramble_controls_frame.pack(fill=tk.X, pady=(0, 5))

        ttk.Button(scramble_controls_frame, text="New Cube", command=self.new_cube).grid(row=0, column=0, padx=5,
                                                                                         pady=5, sticky="ew")
        ttk.Button(scramble_controls_frame, text="Scramble", command=self.scramble_cube, style="Accent.TButton").grid(
            row=0, column=1, padx=5, pady=5, sticky="ew")

        custom_scramble_entry = ttk.Entry(scramble_controls_frame, textvariable=self.custom_scramble_var)
        custom_scramble_entry.grid(row=1, column=0, columnspan=2, padx=5, pady=5, sticky="ew")
        ttk.Button(scramble_controls_frame, text="Use Custom Scramble", command=self.custom_scramble).grid(row=2,
                                                                                                           column=0,
                                                                                                           columnspan=2,
                                                                                                           padx=5,
                                                                                                           pady=5,
        ...
        self.scramble_label = ttk.Label(scramble_controls_frame, textvariable=self.scramble_label_var, wraplength=280,
                                        justify=tk.CENTER)
        self.scramble_label.grid(row=3, column=0, columnspan=2, pady=5)

        scramble_controls_frame.columnconfigure(0, weight=1)
        scramble_controls_frame.columnconfigure(1, weight=1)

        # Solve Controls
        solve_frame = ttk.LabelFrame(right_frame, text="Solve", padding="10")
        solve_frame.pack(fill=tk.X, pady=5)

        ttk.Button(solve_frame, text="Solve Cube", command=self.solve_cube, style="Accent.TButton").pack(fill=tk.X,
                                                                                                         pady=5)

        step_solve_frame = ttk.Frame(solve_frame)
        step_solve_frame.pack()
        ttk.Button(step_solve_frame, text="Cross", command=lambda: self.step_solve("cross")).pack(side=tk.LEFT, padx=2)
        ttk.Button(step_solve_frame, text="F2L", command=lambda: self.step_solve("F2L")).pack(side=tk.LEFT, padx=2)
        ttk.Button(step_solve_frame, text="OLL", command=lambda: self.step_solve("OLL")).pack(side=tk.LEFT, padx=2)
        ttk.Button(step_solve_frame, text="PLL", command=lambda: self.step_solve("PLL")).pack(side=tk.LEFT, padx=2)

        # Solution Display
        solution_text_frame = ttk.LabelFrame(right_frame, text="Solution", padding=(10, 5))
        solution_text_frame.pack(fill=tk.BOTH, expand=True, pady=5)

        self.solution_text = ScrolledText(solution_text_frame, height=4, wrap=tk.WORD, relief=tk.SUNKEN, borderwidth=1)
        self.solution_text.pack(fill=tk.BOTH, expand=True)

        # Solution Buttons Frame (now separate)
        solution_buttons_frame = ttk.Frame(right_frame)
        solution_buttons_frame.pack(fill=tk.X, pady=(0, 5))
        ttk.Button(solution_buttons_frame, text="Copy Solution", command=self.copy_solution).pack(side=tk.LEFT,
                                                                                                  expand=True,
                                                                                                  fill=tk.X,
                                                                                                  padx=(0, 2))
        ttk.Button(solution_buttons_frame, text="Export to alg.cubing.net", command=self.export_solve).pack(
        ...
        self.total_moves_var = tk.StringVar(value="Total Moves: 0")
        self.cross_moves_var = tk.StringVar(value="Cross: 0")
        self.f2l_moves_var = tk.StringVar(value="F2L: 0")
        self.oll_moves_var = tk.StringVar(value="OLL: 0")
        self.pll_moves_var = tk.StringVar(value="PLL: 0")

        ttk.Label(stats_frame, textvariable=self.total_moves_var, font="-weight bold").grid(row=0, column=0,
                                                                                            columnspan=4, pady=(0, 5))
        ttk.Label(stats_frame, textvariable=self.cross_moves_var).grid(row=1, column=0, padx=5)
        ttk.Label(stats_frame, textvariable=self.f2l_moves_var).grid(row=1, column=1, padx=5)
        ttk.Label(stats_frame, textvariable=self.oll_moves_var).grid(row=1, column=2, padx=5)
        ttk.Label(stats_frame, textvariable=self.pll_moves_var).grid(row=1, column=3, padx=5)
        stats_frame.columnconfigure((0, 1, 2, 3), weight=1)

    # --- Backend Interaction Methods ---

    def update_stats(self):
        self.total_moves_var.set(f"Total Moves: {solution_length}")
        self.cross_moves_var.set(f"Cross: {step_moves_list[0]}")
        self.f2l_moves_var.set(f"F2L: {step_moves_list[1]}")
        self.oll_moves_var.set(f"OLL: {step_moves_list[2]}")
        self.pll_moves_var.set(f"PLL: {step_moves_list[3]}")

    def update_solution_text(self):
        self.solution_text.delete('1.0', tk.END)
        self.solution_text.insert(tk.END, get_moves())

    def new_cube(self):
        global a
        a = make_cube()
        self.scramble_label_var.set("Scramble will be displayed here.")
        self.solution_text.delete('1.0', tk.END)
        self.update_stats()
        self.draw_cube()
    ...
    def scramble_cube(self):
        scramble(25)
        self.scramble_label_var.set(get_scramble())
        self.solution_text.delete('1.0', tk.END)  # Clear previous solution
        self.update_stats()
        self.draw_cube()

    def custom_scramble(self):
        scram_str = self.custom_scramble_var.get()
        if not scram_str or scram_str == "Enter custom scramble...":
            return
        scramble(scram_str)
        self.scramble_label_var.set(get_scramble())
        self.solution_text.delete('1.0', tk.END)
    ...
    def step_solve(self, step):
        ...
        if step == "cross":
            ...
            bOLL()
            simplify_moves()
            step_moves_list[2] = solution_length - (step_moves_list[0] + step_moves_list[1])
        elif step == "PLL":
            bPLL()
            simplify_moves()
            step_moves_list[3] = solution_length - (step_moves_list[0] + step_moves_list[1] + step_moves_list[2])
            assert (isSolved())
    ...
    def copy_solution(self):
        ...
        self.root.clipboard_append(get_moves())

    def export_solve(self):
        sCopy = str.replace(get_scramble(), "'", "-").replace(" ", "_")
        mCopy = str.replace(get_moves(), "'", "-").replace(" ", "_")
        url = f"https://alg.cubing.net/?setup={sCopy}&alg={mCopy}"
    ...
    def get_color(self, element):
        color_map = {'B': "#0045ad", 'W': "white", 'G': "#009b48", 'Y': "#ffd500", 'O': "#ff5900", 'R': "#b90000"}
        return color_map.get(element, 'black')
    ...
    def cube_points(self):
        ...
        return [[[u00p, u01p, u02p], [u10p, u11p, u12p], [u20p, u21p, u22p]],
                [[l00p, l01p, l02p], [l10p, l11p, l12p], [l20p, l21p, l22p]],
                [[r02p, r12p, r22p], [r01p, r11p, r21p], [r00p, r10p, r20p]],
                [[d20p, d21p, d22p], [d10p, d11p, d12p], [d00p, d01p, d02p]]]
    ...
    def draw_cube(self):
        ...
        pts = self.cube_points()

        # Draw Down face (drawn first to be in the background)
        for j in range(3):
            for k in range(3):
                self.canvas.create_polygon(pts[3][j][k], fill=self.get_color(a[4][j][k]), outline="#000", width=2)

        # Draw Up, Front, and Right faces
        for i in range(3):
            for j in range(3):
                for k in range(3):
                    fill_color = self.get_color(a[i][j][k])
                    if self.is_transparent:
                        is_hidden_sticker = (i == 1 and (
                                    (j == 1 and k == 2) or (j == 2 and k == 2) or (j == 2 and k == 1))) or \
                                            (i == 2 and ((j == 1 and k == 2) or (j == 2 and k == 2) or (
                                                        j == 2 and k == 1)))
                        if is_hidden_sticker:
                            fill_color = ""  # Make transparent

                    self.canvas.create_polygon(pts[i][j][k], fill=fill_color, outline="#000", width=2)
